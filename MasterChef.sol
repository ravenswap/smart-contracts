...
import "./libs/IRavenReferral.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

import "./RavenToken.sol";

// MasterChef is the master of Raven. He can make Raven and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once RAVEN is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of RAVENs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. RAVENs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that RAVENs distribution occurs.
        uint256 accRavenPerShare;   // Accumulated RAVENs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The RAVEN TOKEN!
    RavenToken public raven;
    // Dev address.
    ...
    // Initial emission rate: 1 RAVEN per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.1 RAVEN per block.
    ...
    // Raven referral contract address.
    IRavenReferral public ravenReferral;
    ...
    constructor(
        ...
        ravenPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    // View function to see pending RAVENs on frontend.
    function pendingRaven(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRavenPerShare = accRavenPerShare.add(ravenReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ravenReward = multiplier.mul(ravenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        raven.mint(devAddress, ravenReward.div(10));
        raven.mint(address(this), ravenReward);
        pool.accRavenPerShare = pool.accRavenPerShare.add(ravenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(ravenReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            ravenReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(raven)) {
    ...
    // Safe raven transfer function, just in case if rounding error causes pool to not have enough RAVENs.
    ...
    function updateEmissionRate() public {
        ...
        require(ravenPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = ravenPerBlock;
        ...
        if (newEmissionRate >= ravenPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = ravenPerBlock;
        ravenPerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the raven referral contract address by the owner
    function setRavenReferral(IRavenReferral _ravenReferral) public onlyOwner {
        ravenReferral = _ravenReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(ravenReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = ravenReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                raven.mint(referrer, commissionAmount);
